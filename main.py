...
def draw_room( pos, screen):
    ...
    if exits[UP] > 0:
        ...
        top = 0
        pygame.draw.rect(screen, WHITE, ((left,top), (80,SCREEN_HEIGHT//4)), 0)
    if exits[DOWN] > 0 :
        #draw bottom exit
        left = SCREEN_WIDTH//2-40
...
def check_room(pos):
    ...
    if player_pos == wumpus_pos:
        game_over("You soul was taken from the Devil!!!")
...
def shoot_arrow(direction):
    ...
    if hit == True:
        game_over("Your aim was true and you have killed the Devil!")
        pygame.quit()
        sys.exit()
    else:    
...
def print_instructions():
    ...
    '''
                             Escape the Devil!
This is the game of "Escape the Devil".  You have been trapped into a
colorless 20 room cave by the fearsome Devil for your sins. The cave is shaped like a 
dodachedron and the only way out is to kill the Devil.  To that end
you have a bow with one arrow. You might find more arrows from those sacrifices to 
the Devil victims in the cave.  There are other dangers in the cave, 
...
If you are near the Devil you will see the bloodstains on the walls.
...
wumpus_img = pygame.image.load('images/devil.png')
...
mobile_wumpus = True #Set this to true to allow the wumpus to move
...
pygame.display.set_caption("Escape the Devil")
